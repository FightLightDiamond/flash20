...
_t_JWT_SECRET_KEY = _t.Union[_t.Iterable[_t_str_bytes], _t_str_bytes]
...
def _make_keys_list(JWT_SECRET_KEY: _t_JWT_SECRET_KEY) -> _t.List[bytes]:
    if isinstance(JWT_SECRET_KEY, (str, bytes)):
        return [want_bytes(JWT_SECRET_KEY)]

    return [want_bytes(s) for s in JWT_SECRET_KEY]
...
class Signer:
    ...
    :param JWT_SECRET_KEY: The secret key to sign and verify with. Can be a
        list of keys, oldest to newest, to support key rotation.
    :param salt: Extra key to combine with ``JWT_SECRET_KEY`` to distinguish
    ...
    def __init__(
        self,
        JWT_SECRET_KEY: _t_JWT_SECRET_KEY,
        ...
        self.JWT_SECRET_KEYs: _t.List[bytes] = _make_keys_list(JWT_SECRET_KEY)
    ...
    def JWT_SECRET_KEY(self) -> bytes:
        """The newest (last) entry in the :attr:`JWT_SECRET_KEYs` list. This
        is for compatibility from before key rotation support was added.
        """
        return self.JWT_SECRET_KEYs[-1]

    def derive_key(self, JWT_SECRET_KEY: _t_opt_str_bytes = None) -> bytes:
        ...
        :param JWT_SECRET_KEY: A specific secret key to derive from.
            Defaults to the last item in :attr:`JWT_SECRET_KEYs`.

        .. versionchanged:: 2.0
            Added the ``JWT_SECRET_KEY`` parameter.
        """
        if JWT_SECRET_KEY is None:
            JWT_SECRET_KEY = self.JWT_SECRET_KEYs[-1]
        else:
            JWT_SECRET_KEY = want_bytes(JWT_SECRET_KEY)

        if self.key_derivation == "concat":
            return _t.cast(bytes, self.digest_method(self.salt + JWT_SECRET_KEY).digest())
        elif self.key_derivation == "django-concat":
            return _t.cast(
                bytes, self.digest_method(self.salt + b"signer" + JWT_SECRET_KEY).digest()
            )
        elif self.key_derivation == "hmac":
            mac = hmac.new(JWT_SECRET_KEY, digestmod=self.digest_method)
    ...
    def verify_signature(self, value: _t_str_bytes, sig: _t_str_bytes) -> bool:
        ...
        for JWT_SECRET_KEY in reversed(self.JWT_SECRET_KEYs):
            key = self.derive_key(JWT_SECRET_KEY)
